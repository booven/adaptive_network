...
由于本网络对骨干网络的特殊要求，故在part部分进行了修改。请注意，为了与其他方法更好的比对，我们并没有改动任何block里面的内容。
"""
import os
import argparse
...
class Xception(nn.Module):
    ...
    def classifier(self, features):
        x = self.relu(features)   #   x torch.Size([10, 2048, 8, 8])
        # x = F.adaptive_avg_pool2d(x, (1, 1))    # x torch.Size([10, 2048, 1, 1])
        x = x.view(x.size(0), -1)     #   x torch.Size([10, 2048])
        out = self.last_linear(x)    #   out torch.Size([10, 1])
        return out, features
